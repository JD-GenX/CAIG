...
from transformers import AutoConfig, AutoModelForCausalLM, LlamaConfig, LlamaModel, LlamaForCausalLM, LlamaForSequenceClassification
...
class LlavaLlamaForClass(LlamaForSequenceClassification, LlavaMetaForCausalLM):
    config_class = LlavaConfig
    ...
    def forward(
        ...
        if inputs_embeds is None:
            (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds, labels) = self.prepare_inputs_labels_for_multimodal_class(
    ...
    def generate(
        ...
        **kwargs,
        ...
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _) = self.prepare_inputs_labels_for_multimodal_class(
    ...
    def generate_g(
        ...
        **kwargs,
        ...
        if images is not None:
            (inputs, position_ids, attention_mask, past_key_values, inputs_embeds, labels) = self.prepare_inputs_labels_for_multimodal_class(
                inputs, position_ids, attention_mask, None, None, images, image_sizes=image_sizes
            )
        else:
            inputs_embeds = self.get_model().embed_tokens(inputs)

        return self.generate_w_logits(
            input_ids=inputs,
            position_ids=position_ids,
            attention_mask=attention_mask,
            inputs_embeds=inputs_embeds,
        ...
        **model_kwargs,
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                ...
                return SampleDecoderOnlyOutput_wlogits(
                    sequences=input_ids, scores=scores, attentions=decoder_attentions, hidden_states=decoder_hidden_states, logits=outputs.logits
                )
        else:
            return input_ids
...
AutoModelForCausalLM.register(LlavaConfig, LlavaLlamaForClass)
