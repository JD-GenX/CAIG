...
class LlamaForSequenceClassification(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        print("LlamaForSequenceClassification config", config)
        self.num_labels = config.num_labels
        self.model = LlamaModel(config)
        self.score = nn.Linear(config.hidden_size, self.num_labels, bias=False)
        drop_prob = 0.5
        hidden_dims_img = [512, 2]
        self.sku_ctr_linear_fc = nn.Sequential(
            nn.Linear(4096, hidden_dims_img[0]),
        ...
        print("父类方法 sku_ctr_linear_fc def")
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        print("pooled_ctr_fc_out shape", pooled_ctr_fc_out.shape)
