...
class DDIMSchedulerExtended(DDIMScheduler):
  ...
  def _get_variance_new(self, timestep, prev_timestep): #new
  ...
  def _left_broadcast_new(self, t, shape): #new
  ...
  def ddim_step_with_logprob(   #new
    self,
    ...
    # 2. compute alphas, betas
    # print(self.alphas_cumprod.size(),timestep.size())
    alpha_prod_t = self.alphas_cumprod.gather(0, timestep)
    alpha_prod_t_prev = torch.where(
        prev_timestep >= 0, self.alphas_cumprod.gather(0, prev_timestep), self.final_alpha_cumprod
    )
    alpha_prod_t = self._left_broadcast_new(alpha_prod_t, sample.shape).to(sample.device)
    alpha_prod_t_prev = self._left_broadcast_new(alpha_prod_t_prev, sample.shape).to(sample.device)
    ...
    # 5. compute variance: "sigma_t(η)" -> see formula (16)
    # σ_t = sqrt((1 − α_t−1)/(1 − α_t)) * sqrt(1 − α_t/α_t−1)
    variance = self._get_variance_new(timestep, prev_timestep)
    std_dev_t = eta * variance ** (0.5)
    std_dev_t = self._left_broadcast_new(std_dev_t, sample.shape).to(sample.device)
